...
time it was formed. Note that what is stored is NOT necessarily
the amiga.grp ID (or other specified IDkey), but the index of that 
halo in the tangos database. The amiga.grp ID (or other specified IDkey)
can be backed out via tangos with sim[stepnum][halonum].finder_id.
...
def FindHaloStars(dsnames):
    ...
    filename = outpath+cursim+'_stardata_'+dsnames[0].split('.')[-1]+'.h5'
